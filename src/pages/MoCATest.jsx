...
const MoCATest = () => {
    ...
    if (isFinished) {
        const score = calculateScore(answers, mocaQuestions);
        const result = interpretMOCA(score);

        return (
            <div className="test-v2-container animate-fade-up">
                <div className="card-modern results-summary-v2">
                    <div className="completion-top">
                        <div className="lottie-mock-success">
                            <CheckCircle2 size={60} strokeWidth={3} className="text-secondary" />
                        </div>
                        ...
                        <div className="circular-progress-v2">
                            <svg viewBox="0 0 100 100">
                                ...
                                <circle
                                    ...
                                    strokeWidth="10"
                                    fill="none"
                                    strokeDasharray={`${(score / 30) * 283} 283`}
                                    strokeLinecap="round"
                                    style={{ transition: 'stroke-dasharray 1.5s ease-out' }}
                                />
                            </svg>
                            <div className="center-score">
                                <span className="sc-big">{score}</span>
                                <span className="sc-sm">Score</span>
                            </div>
                        </div>

                        <div className="result-badge-v2" style={{ background: `${result.color}15`, color: result.color }}>
                            <Activity size={24} />
                            <div>
                                <h3>{result.level} Screening Result</h3>
                    ...
                    <div className="action-footer-v2">
                        <button className="btn-primary-lg" onClick={() => window.location.href = '/reports'}>
                            View Clinical Summary <ArrowRight size={20} />
                        </button>
                        <button className="btn-link-v2" onClick={() => window.location.href = '/dashboard'}>
                            Return to Dashboard
                        </button>
                    </div>
                </div>
          ...
          .result-badge-v2 { 
            display: flex; align-items: center; gap: 20px; padding: 24px; border-radius: var(--radius-md); 
          ...
          .btn-link-v2:hover { color: var(--primary); }
        `}</style>
            </div>
        );
    }
    ...
    return (
        <div className="test-player-v2 container animate-fade-up">
            <header className="player-header-v2">
                <div className="player-meta">
                    <div className="meta-icon-v2" style={{ background: '#f0fdf4', color: 'var(--secondary)' }}>
                        <Activity size={24} />
                    </div>
                    <div>
            ...
            <div className="player-body-v2">
                ...
                <div className="player-nav-v2">
                    ...
                    <div className="player-help">
                        <AlertCircle size={16} /> Assistance required?
                    </div>
                    <button
                        className="btn-text-v2"
                        onClick={() => setCurrentStep(Math.min(mocaQuestions.length - 1, currentStep + 1))}
                    >
        ...
        .question-card-v2 { padding: 48px; min-height: 500px; display: flex; flex-direction: column; }
        .question-intro { display: flex; align-items: center; gap: 8px; font-size: 0.85rem; color: var(--text-muted); font-weight: 700; text-transform: uppercase; margin-bottom: 24px; }
        .q-text-v2 { font-size: 2rem; line-height: 1.3; margin-bottom: 48px; color: var(--text-main); }
        
        .options-stack-v2 { display: grid; gap: 16px; margin-top: auto; }
        .opt-tile-v2 { 
          display: flex; align-items: center; gap: 20px; padding: 16px 24px; background: white; border: 2px solid var(--surface-alt); border-radius: 16px; 
          text-align: left; transition: var(--transition); cursor: pointer; font-size: 1.1rem; font-weight: 600; color: var(--text-main);
        }
        .opt-tile-v2:hover { border-color: var(--secondary); background: var(--surface-alt); }
        .opt-tile-v2.selected-secondary { border-color: var(--secondary); background: #f0fdf4; box-shadow: 0 4px 12px rgba(111, 207, 151, 0.1); }
        .opt-indicator { 
          width: 32px; height: 32px; border-radius: 50%; background: #f1f5f9; display: flex; align-items: center; justify-content: center; 
          font-weight: 800; color: var(--text-muted); font-size: 0.9rem; flex-shrink: 0;
        }
        .opt-tile-v2.selected-secondary .opt-indicator { background: var(--secondary); color: white; }
