...
const runCognitiveInference = (age, sex, educYears, mocaScore, cdrSum = 0) => {
  return new Promise((resolve, reject) => {
    ...
    const args = [
      ...
      '--age',  String(age),
      '--sex',  String(sex),
      '--educ', String(educYears),
      '--moca', String(mocaScore),
      '--cdr',  String(cdrSum),
    ];
    ...
    execFile(pythonPath, args, { timeout: 30000 }, (err, stdout, stderr) => {
      if (err) {
        ...
        return resolve(fallbackInference(age, sex, educYears, mocaScore, cdrSum));
      }
      try {
...
const fallbackInference = (age, sex, educYears, mocaScore, cdrSum) => {
  ...
  const risk_level = riskProb < 0.25 ? 'Low' : riskProb < 0.55 ? 'Moderate' : 'High';
...
exports.predictCognitiveRisk = async (req, res) => {
  try {
    ...
    if (!moca_score && moca_score !== 0) {
      return res.status(400).json({ success: false, message: 'moca_score is required.' });
    }
    if (!age) {
      return res.status(400).json({ success: false, message: 'Date of birth must be set in your profile to use this feature.' });
    }
    ...
    res.status(200).json({
      ...
      data: {
        ...result,
        user_demographics: { age, sex: user.gender, education_years: educYears },
        result_id: saved?.id || null,
      },
    });
  } catch (err) {
    console.error('[ML Predict Error]', err);
    res.status(500).json({ success: false, message: 'Internal server error during ML inference.' });
  }
};
